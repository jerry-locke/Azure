...
if ($parameters.parameters.sasToken -ne $null)
{
    ...
    $parameters | ConvertTo-Json -depth 100 | Out-File $parametersFilePath

    Select-AzureRmSubscription -SubscriptionName $subscriptionName;
}
}

$seqParameters = Get-Content -Raw -Path $parametersFilePath | ConvertFrom-Json
$sequenceVersion = [guid]::NewGuid()

$sequenceVersionValue = @"
{
"value": "$sequenceVersion"
}
"@

$seqParameters.parameters | Add-Member -Name "sequenceVersion" -Value (ConvertFrom-Json $sequenceVersionValue) -MemberType NoteProperty -Force
$seqParameters | ConvertTo-Json -depth 100 | Out-File $parametersFilePath
